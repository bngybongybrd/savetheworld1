...
def load_user(row_id):
  ...
  curs = conn.cursor()
  curs.execute("SELECT rowid, * from user_database where rowid = (?)",
               [row_id])
  lu = curs.fetchone()
  if lu is None:
    return None
...
def register():
  ...
  if form.validate_on_submit():
    sqlite_code.add_user(form.username.data, form.email.data,
                         form.password.data)
...
def login():
  ...
  form = LoginForm()
  if form.validate_on_submit():
    if sqlite_code.email_lookup(form.email.data) and sqlite_code.pw_check(
...
def new_post():
  ...
  if form.validate_on_submit():
    ...
    sqlite_code.add_post(user[1], form.title.data, form.content.data,
